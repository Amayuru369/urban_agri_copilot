...
def _generate_pdf(
    ...
    if savings_items:
        # Multi-crop basket shape: {"items": [...], "currency": ..., "total_estimated_savings": ...}
        currency = savings_data.get("currency", "LKR")
        savings_rows = [
            [
                _cell("Crop", normal_style, bold=True),
                _cell("Yield", normal_style, bold=True),
                _cell("Retail price", normal_style, bold=True),
                _cell("Savings", normal_style, bold=True),
            ]
        ]
        for item in savings_items:
            item_currency = item.get("currency", currency)
            savings_rows.append([
                _cell(_val(item.get("crop_name")), normal_style),
                _cell(f"{_val(item.get('expected_yield_kg'))} kg", normal_style),
                _cell(f"{_val(item.get('retail_price_per_kg'))} {item_currency}/kg", normal_style),
                _cell(f"{_val(item.get('estimated_savings'))} {item_currency}", normal_style),
            ])
        ...
        savings_rows.append([
            _cell("Grand total", normal_style, bold=True),
            _cell("", normal_style),
            _cell("", normal_style),
            _cell(f"{_val(total_savings)} {currency}", normal_style, bold=True),
        ])
        savings_table = Table(savings_rows, colWidths=[150, 90, 130, 100])
        ...
        # Legacy single-crop shape — unchanged from before.
        savings_rows = [
...
def _generate_markdown(
    ...
    if savings_items:
        ...
        for item in savings_items:
            item_currency = item.get("currency", currency)
            lines.append(
                f"- **{_val(item.get('crop_name'))}:** {_val(item.get('expected_yield_kg'))} kg "
