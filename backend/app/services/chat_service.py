...
def _build_garden_context(db: Session) -> str:
...
async def ask_garden_copilot(user_message: str, history: list[dict], db: Session) -> str:
    """Generate a context-aware reply using Gemini with live garden state."""
    garden_context = _build_garden_context(db)
